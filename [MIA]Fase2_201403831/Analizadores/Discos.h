...
char tFormateo[10];
//Comandos a utilizar
int c_mkdisk = 0; int c_rmdisk = 0;
int c_fdisk = 0;int c_mount = 0;
int c_umount = 0;int c_mbr = 0;
int c_disk = 0;int c_exec = 0;
int c_mkfs = 0;int c_login = 0;
int c_logout = 0;int c_tunefs = 0;
int c_recovery = 0;int c_loss= 0;
int c_mkgrp = 0; int c_rmgrp = 0;
int c_mkusr = 0; int c_rmusr = 0;
int c_mkdir = 0; int c_mkfile = 0;
int c_chgrp = 0; int c_ren = 0;
int c_cat = 0; int c_mv = 0;
int c_chmod = 0; int c_chown = 0;
int c_edit = 0;
...
void InicializarComandos()
{
 ...
 c_mkfs = 0; c_login = 0; c_logout = 0; c_tunefs = 0; c_recovery = 0; c_loss = 0; c_mkgrp = 0;
 c_rmgrp = 0; c_mkusr = 0; c_rmusr = 0; c_mkdir = 0; c_mkfile = 0; c_chgrp = 0; c_ren = 0;
 c_mv = 0; c_chmod = 0; c_chown = 0; c_edit = 0;
}

int CantLetras(char cad[])
{
    int exito = 0;

    if(strlen(cad) <= 10) exito = 1;
...
{
    if(c_mkdisk == 1) comando_mkdisk();
    else if(c_rmdisk == 1)comando_rmdisk();
    else if(c_fdisk == 1)comando_fdisk();
    else if(c_mount == 1)comando_mount();
    else if(c_umount == 1)comando_mount();
    else if(c_mkfs == 1)comando_mkfs();
    else if(c_login == 1)comando_login();
    else if(c_logout == 1)comando_logout();
    else if(c_tunefs == 1)comando_tunefs();
    else if(c_recovery == 1)comando_recovery();
    else if(c_loss == 1) comando_loss();
    else if(c_mkgrp == 1) comando_mkgrp();
    else if(c_rmgrp == 1) comando_rmgrp();
    else if(c_mkusr == 1) comando_mkusr();
    else if(c_rmusr == 1) comando_rmusr();
    else if(c_mkdir == 1) comando_mkdir();
    else if(c_mkfile == 1) comando_mkfile();
    else if(c_chgrp == 1) comando_chgrp();
    else if(c_ren == 1) comando_ren();
    else if(c_mv == 1) comando_mv();
    else if(c_chmod == 1) comando_chmod();
    else if(c_chown == 1) comando_chown();
    else if(c_edit == 1) comando_edit();
    else if(c_exec == 1) script_exec();
}
...
if(tamanio!=-1)
{
    ...
    datos_extend.part_logic_1.part_status = 'i';
    datos_extend.part_logic_2.part_status = 'i';
    datos_extend.part_logic_3.part_status = 'i';
    datos_extend.part_logic_4.part_status = 'i';
    datos_extend.part_logic_5.part_status = 'i';
...
int ExisteNombreParticion(MBR disco)
{
...
int TamTotalParticiones(MBR disco)
{
    int totalParticiones;

        if(disco.mbr_partition_1.part_status == 'a') totalParticiones += disco.mbr_partition_1.part_size;
        if(disco.mbr_partition_2.part_status == 'a') totalParticiones += disco.mbr_partition_2.part_size;
        if(disco.mbr_partition_3.part_status == 'a') totalParticiones += disco.mbr_partition_3.part_size;
        if(disco.mbr_partition_4.part_status == 'a') totalParticiones += disco.mbr_partition_4.part_size;

    return totalParticiones;
}

int PosicionEBR(MBR disco)
{
    int posicion = sizeof(MBR) + 1;

    if(disco.mbr_partition_1.part_status == 'a')
    {
        posicion = sizeof(MBR) + 1;
    }
    if(disco.mbr_partition_2.part_status == 'a')
    {
        posicion = disco.mbr_partition_1.part_start + disco.mbr_partition_1.part_size + 1;
    }
    if(disco.mbr_partition_3.part_status == 'a')
    {
        posicion = disco.mbr_partition_2.part_start + disco.mbr_partition_2.part_size + 1;
    }
    if(disco.mbr_partition_4.part_status == 'a')
    {
        posicion = disco.mbr_partition_3.part_start + disco.mbr_partition_3.part_size + 1;
    }

    return posicion;
}

int NumParticionLogica(MBR disco)
{
    int Part = 1;

    if(disco.mbr_partition.part_logic_1.part_status == 'a') Part = 1;
    if(disco.mbr_partition.part_logic_2.part_status == 'a') Part = 2;
    if(disco.mbr_partition.part_logic_3.part_status == 'a') Part = 3;
    if(disco.mbr_partition.part_logic_4.part_status == 'a') Part = 4;
    if(disco.mbr_partition.part_logic_5.part_status == 'a') Part = 5;
...
void fdisk_crear()
{
    ...
    if(tipo == 'L')
    {
        ...
        if(Validar == NULL) { printf("El path indicado no es correcto,\n\n"); return;}
        fclose(Validar);
        int Existente = 0;
        MBR disco;
        disco = returnMBR(fichero);
        int totalExtendida = 0;
        if(disco.mbr_partition.part_status != 'a')
        { printf("Debe crear primero una partición extendida.\n\n"); return;}
        totalExtendida = disco.mbr_partition.part_size;
        if(totalExtendida < cBytes) {printf("No hay suficiente espacio en la partición extendida.\n\n"); return;}
        int nParticion = NumParticionLogica(disco);

        part_logic partiLogica;
        partiLogica.part_size = cBytes;
        partiLogica.part_start = 1;
        partiLogica.part_status = 'a';
        strcpy(partiLogica.part_name,nombre);
        if(strcmp(ajustefit,"WF") == 0)
        { strcpy(partiLogica.part_fit,"WF"); }
        else if(strcmp(ajustefit,"BF") == 0)
        { strcpy(partiLogica.part_fit,"BF"); }
        else
        {strcpy(partiLogica.part_fit,"FF");}
        switch(nParticion)
        {
            case 1: disco.mbr_partition.part_logic_1 = partiLogica;
            break;
            case 2: disco.mbr_partition.part_logic_2 = partiLogica;
            break;
            case 3: disco.mbr_partition.part_logic_3 = partiLogica;
            break;
            case 4: disco.mbr_partition.part_logic_4 = partiLogica;
            break;
            case 5: disco.mbr_partition.part_logic_5 = partiLogica;
            break;
        }
        printf("%c\n\n",disco.mbr_partition.part_logic_1.part_status);
    ...
    else if(tipo == 'E')
    {
        ...
        totalParticiones = TamTotalParticiones(disco);
        if(disco.mbr_partition.part_status == 'a') nombreExistente = 1;
        nombreExistente = ExisteNombreParticion(disco);

        totalParticiones = disco.mbr_tamanio - totalParticiones;

        if(totalParticiones > cBytes)
        {
            part_extended ParticioE;
            ParticioE.part_status = 'a';
            ParticioE.part_size = cBytes;
            if(strcmp(ajustefit,"WF") == 0)
                { strcpy(ParticioE.part_fit,"WF"); }
                else if(strcmp(ajustefit,"BF") == 0)
                { strcpy(ParticioE.part_fit,"BF"); }
                else
                {
                    strcpy(ParticioE.part_fit,"FF");
                }

            if(nombreExistente == 1) { printf("El nombre de la partición ya existe o ya existe una partición extendida.\n\n"); return; }
            strcpy(ParticioE.part_name,nombre);

            ParticioE.part_start = PosicionEBR(disco);
            disco.mbr_partition = ParticioE;
            reemplazoMBR(fichero,disco);
            printf("Particion extendida creada exitosamente.\n\n");
        }
        else
        {
    ...
    else
    {
            ...
            if(totalParticiones > cBytes)
            {
                ...
                if(strcmp(ajustefit,"WF") == 0)
                { strcpy(particioP.part_fit,"WF"); }
                else if(strcmp(ajustefit,"BF") == 0)
                { strcpy(particioP.part_fit,"BF"); }
                else
                {
                ...
                if(nombreExistente != 1)
                {
                    ...
                    if(part1 == 0)
                    {
                        particioP.part_start = sizeof(MBR) + 1;
                        disco.mbr_partition_1 = particioP;
                    }
                    else if(part2 == 0)
                    {
                        particioP.part_start = disco.mbr_partition_1.part_start + disco.mbr_partition_1.part_size + 1;
                        disco.mbr_partition_2 = particioP;
                    }
                    else if(part3 == 0)
                    {
                        particioP.part_start = disco.mbr_partition_2.part_start + disco.mbr_partition_2.part_size + 1;
                        disco.mbr_partition_3 = particioP;
                    }
                    else if(part4 == 0)
                    {
                        particioP.part_start = disco.mbr_partition_3.part_start + disco.mbr_partition_3.part_size + 1;
...
ComillasCadId();
    ...
    if(strcmp(tReport,"mbr") == 0) reporte_mbr();
    else if(strcmp(tReport,"disk") == 0) reporte_disk();
    else if(strcmp(tReport,"inode") == 0) reporte_inode();
    else if(strcmp(tReport,"block") == 0) reporte_block();
    else if(strcmp(tReport,"sb") == 0) reporte_sb();
    else if(strcmp(tReport,"bm_inode") == 0) reporte_bm_inode();
    else if(strcmp(tReport,"bm_block") == 0) reporte_bm_block();
    else if(strcmp(tReport,"journaling") == 0) reporte_journaling();
    else if(strcmp(tReport,"tree") == 0) reporte_tree();
    else if(strcmp(tReport,"file")==0) reporte_file();
    else  printf("Tipo de reporte no identificado.\n\n");
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if(disco.mbr_partition.part_status == 'a')
    {
        sprintf(hola,"<TR><TD>part_status_Extended</TD><TD>%c</TD></TR>\n <TR><TD>part_type_Extended</TD><TD>Extendida</TD></TR>\n",disco.mbr_partition.part_status);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
        sprintf(hola,"<TR><TD>part_fit_Extended</TD><TD>%s</TD></TR>\n <TR><TD>part_start_Extended</TD><TD>%d</TD></TR>\n",disco.mbr_partition.part_fit, disco.mbr_partition.part_start);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
        sprintf(hola,"<TR><TD>part_size_Extended</TD><TD>%d</TD></TR>\n <TR><TD>part_name_Extended</TD><TD>%s</TD></TR>\n",disco.mbr_partition.part_size, disco.mbr_partition.part_name);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
    }

    if(disco.mbr_partition.part_logic_1.part_status == 'a')
    {
        sprintf(hola,"<TR><TD>part_status_Logica1</TD><TD>%c</TD></TR>\n <TR><TD>part_type_Logica1</TD><TD>Logica</TD></TR>\n",disco.mbr_partition.part_logic_1.part_status);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
        sprintf(hola,"<TR><TD>part_fit_Logica1</TD><TD>%s</TD></TR>\n <TR><TD>part_start_Logica1</TD><TD>%d</TD></TR>\n",disco.mbr_partition.part_logic_1.part_fit, disco.mbr_partition.part_logic_1.part_start);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
        sprintf(hola,"<TR><TD>part_size_Logica1</TD><TD>%d</TD></TR>\n <TR><TD>part_name_Logica1</TD><TD>%s</TD></TR>\n",disco.mbr_partition.part_logic_1.part_size, disco.mbr_partition.part_logic_1.part_name);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
    }

    if(disco.mbr_partition.part_logic_2.part_status == 'a')
    {
        sprintf(hola,"<TR><TD>part_status_Logica</TD><TD>%c</TD></TR>\n <TR><TD>part_type_Logica1</TD><TD>Logica</TD></TR>\n",disco.mbr_partition.part_logic_2.part_status);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
        sprintf(hola,"<TR><TD>part_fit_Logica</TD><TD>%s</TD></TR>\n <TR><TD>part_start_Logica1</TD><TD>%d</TD></TR>\n",disco.mbr_partition.part_logic_2.part_fit, disco.mbr_partition.part_logic_2.part_start);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
        sprintf(hola,"<TR><TD>part_size_Logica</TD><TD>%d</TD></TR>\n <TR><TD>part_name_Logica1</TD><TD>%s</TD></TR>\n",disco.mbr_partition.part_logic_2.part_size, disco.mbr_partition.part_logic_2.part_name);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
    }

    if(disco.mbr_partition.part_logic_3.part_status == 'a')
    {
        sprintf(hola,"<TR><TD>part_status_Logica</TD><TD>%c</TD></TR>\n <TR><TD>part_type_Logica1</TD><TD>Logica</TD></TR>\n",disco.mbr_partition.part_logic_3.part_status);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
        sprintf(hola,"<TR><TD>part_fit_Logica</TD><TD>%s</TD></TR>\n <TR><TD>part_start_Logica1</TD><TD>%d</TD></TR>\n",disco.mbr_partition.part_logic_3.part_fit, disco.mbr_partition.part_logic_3.part_start);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
        sprintf(hola,"<TR><TD>part_size_Logica</TD><TD>%d</TD></TR>\n <TR><TD>part_name_Logica1</TD><TD>%s</TD></TR>\n",disco.mbr_partition.part_logic_3.part_size, disco.mbr_partition.part_logic_3.part_name);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
    }

    if(disco.mbr_partition.part_logic_4.part_status == 'a')
    {
        sprintf(hola,"<TR><TD>part_status_Logica</TD><TD>%c</TD></TR>\n <TR><TD>part_type_Logica1</TD><TD>Logica</TD></TR>\n",disco.mbr_partition.part_logic_4.part_status);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
        sprintf(hola,"<TR><TD>part_fit_Logica</TD><TD>%s</TD></TR>\n <TR><TD>part_start_Logica1</TD><TD>%d</TD></TR>\n",disco.mbr_partition.part_logic_4.part_fit, disco.mbr_partition.part_logic_4.part_start);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
        sprintf(hola,"<TR><TD>part_size_Logica</TD><TD>%d</TD></TR>\n <TR><TD>part_name_Logica1</TD><TD>%s</TD></TR>\n",disco.mbr_partition.part_logic_4.part_size, disco.mbr_partition.part_logic_4.part_name);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
    }

    if(disco.mbr_partition.part_logic_5.part_status == 'a')
    {
        sprintf(hola,"<TR><TD>part_status_Logica</TD><TD>%c</TD></TR>\n <TR><TD>part_type_Logica1</TD><TD>Logica</TD></TR>\n",disco.mbr_partition.part_logic_5.part_status);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
        sprintf(hola,"<TR><TD>part_fit_Logica</TD><TD>%s</TD></TR>\n <TR><TD>part_start_Logica1</TD><TD>%d</TD></TR>\n",disco.mbr_partition.part_logic_5.part_fit, disco.mbr_partition.part_logic_5.part_start);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
        sprintf(hola,"<TR><TD>part_size_Logica</TD><TD>%d</TD></TR>\n <TR><TD>part_name_Logica1</TD><TD>%s</TD></TR>\n",disco.mbr_partition.part_logic_5.part_size, disco.mbr_partition.part_logic_5.part_name);
        fprintf(repMBR,hola);
        memset(hola,0,9999);
    }
...
void reporte_disk()
{
    ...
    else
    {
        ...
        if(disco.mbr_partition_3.part_status != 'a' && seguir == 1)
        {
            memset(hola,0,9999);
            particiones++;
            sprintf(hola,"<TD rowspan=\"%d\">       Libre       </TD>",particiones);
        ...
        if(disco.mbr_partition_4.part_status != 'a' && seguir == 1)
        {
            memset(hola,0,9999);
            particiones++;
            sprintf(hola,"<TD rowspan=\"%d\">       Libre       </TD>",particiones);
    ...
    {
        if(disco.mbr_partition.part_logic_1.part_status == 'a' ||
        disco.mbr_partition.part_logic_2.part_status == 'a' ||
        disco.mbr_partition.part_logic_3.part_status == 'a' ||
        disco.mbr_partition.part_logic_4.part_status == 'a' ||
        disco.mbr_partition.part_logic_5.part_status == 'a' )
        {
            int seguir = 0;
            if(disco.mbr_partition.part_logic_1.part_status == 'a')
            {
                memset(hola,0,9999); strcpy(hola,"<TD> <TABLE><TR><TD>Extendida</TD></TR>"); fprintf(repMBR,hola); memset(hola,0,9999);
                memset(hola,0,9999); strcpy(hola," <TR><TD>Logica</TD></TR></TABLE></TD>"); fprintf(repMBR,hola); memset(hola,0,9999);
                seguir = 1;
            }
            else
            {
                memset(hola,0,9999); strcpy(hola,"<TD> <TABLE><TR><TD>Extendida</TD></TR>"); fprintf(repMBR,hola); memset(hola,0,9999);
                memset(hola,0,9999); strcpy(hola," <TR><TD>Libre/TD></TR></TABLE></TD>"); fprintf(repMBR,hola); memset(hola,0,9999);
                seguir = 1;
            }
            if(disco.mbr_partition.part_logic_2.part_status == 'a' && seguir == 1)
            {
                memset(hola,0,9999); strcpy(hola,"<TD> <TABLE><TR><TD>Extendida</TD></TR>"); fprintf(repMBR,hola); memset(hola,0,9999);
                memset(hola,0,9999); strcpy(hola," <TR><TD>Logica</TD></TR></TABLE></TD>"); fprintf(repMBR,hola); memset(hola,0,9999);
                seguir = 2;
            ...
            if(disco.mbr_partition.part_logic_3.part_status == 'a'&& seguir == 2)
            {
                memset(hola,0,9999); strcpy(hola,"<TD> <TABLE><TR><TD>Extendida</TD></TR>"); fprintf(repMBR,hola); memset(hola,0,9999);
                memset(hola,0,9999); strcpy(hola," <TR><TD>Logica</TD></TR></TABLE></TD>"); fprintf(repMBR,hola); memset(hola,0,9999);
                seguir = 3;
            ...
            if(disco.mbr_partition.part_logic_4.part_status == 'a'&& seguir == 3)
            {
                memset(hola,0,9999); strcpy(hola,"<TD> <TABLE><TR><TD>Extendida</TD></TR>"); fprintf(repMBR,hola); memset(hola,0,9999);
                memset(hola,0,9999); strcpy(hola," <TR><TD>Logica</TD></TR></TABLE></TD>"); fprintf(repMBR,hola); memset(hola,0,9999);
                seguir = 4;
            ...
            if(disco.mbr_partition.part_logic_5.part_status == 'a'&& seguir == 4)
            {
                memset(hola,0,9999); strcpy(hola,"<TD> <TABLE><TR><TD>Extendida</TD></TR>"); fprintf(repMBR,hola); memset(hola,0,9999);
                memset(hola,0,9999); strcpy(hola," <TR><TD>Logica</TD></TR></TABLE></TD>"); fprintf(repMBR,hola); memset(hola,0,9999);
            }
        ...
        {
        memset(hola,0,9999); strcpy(hola,"<TD> <TABLE><TR><TD>Extendida</TD></TR></TABLE> </TD>"); fprintf(repMBR,hola); memset(hola,0,9999);
        }
    }
    //////////////////////////////////////////////////////////////////////
